...
export default function Header({ demoMode = false, demoToken }: HeaderProps) {
  const logoHref = demoMode && demoToken ? `/demo/${demoToken}` : "/skills";
    ...
    <header className="border-b-2 border-gray-200 bg-white sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-6 flex items-start justify-between gap-4">
        ...
        {demoMode ? (
          <Link
            href="/login"
            className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-lg transition"
          >
            <LogIn size={15} />
            Sign in
          </Link>
